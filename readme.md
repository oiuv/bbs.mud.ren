...
论坛首次访问默认使用与 mud.ren 主站一致的深色暖橙主题。导航右侧的「浅色 / 深色」按钮可随时切换，手机端不需要展开菜单。切换不会重新加载页面或清空正在编辑的内容。

选择保存在当前浏览器的 `mudren-theme` 本地存储项中，刷新和切换页面后保留，并同步到同源的其他标签页；不随账号跨设备同步。浏览器禁止本地存储时，仍可在当前页面切换。系统的深浅偏好不会覆盖用户选择或默认深色。

主题覆盖论坛自身页面、Markdown、编辑器和弹窗；帖子图片保持原色，验证码和 Dify 等外部嵌入内容由其服务提供方决定外观。发布时需完整上传 `dist/`，其中 `theme.js` 用于应用加载前恢复主题。
