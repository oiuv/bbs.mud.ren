<template>
  <div class="comments" name="comments">
    ...
    <div class="box box-flush" v-if="item.content && item.content.body">
      <div class="border-bottom box-body py-2" :class="{'animated flash': $route.hash === '#comment-' + item.id}" v-for="(item,index) in comments.data" :key="item.id" :id="'comment-' + item.id" :name="'comment-' + item.id">
